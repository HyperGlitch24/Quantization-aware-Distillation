...
def load_checkpoint(ckpt_path, student_path, optimizer, scheduler, device):
    ...
    # re-apply LoRA — checkpoint saved adapter weights in ckpt_path
    from peft import PeftModel
    student = PeftModel.from_pretrained(base, ckpt_path)
    # this loads the adapter weights from ckpt_path/adapter_model.bin
    # which save_checkpoint correctly saved via student.save_pretrained(ckpt_path)
...
Ckpt_dir = "./poc_checkpoints_1.3B_LoRA_deepseek_6.7B"
...
if latest_ckpt is not None:
    ...
    student = PeftModel.from_pretrained(base, latest_ckpt, is_trainable=True)
    student.enable_input_require_grads()
    student.gradient_checkpointing_enable(
        gradient_checkpointing_kwargs={"use_reentrant": False}
    )
    ...
    # optimizer built from the ACTUAL student
    optimizer = AdamW(
    ...
    scheduler.load_state_dict(torch.load(
        os.path.join(latest_ckpt, "scheduler.pt"), weights_only=False))
    ...
    rng = torch.load(os.path.join(latest_ckpt, "rng_states.pt"), weights_only=False)
    torch.set_rng_state(rng["rng_torch"])
    torch.cuda.set_rng_state(rng["rng_cuda"])
    np.random.set_state(rng["rng_numpy"])
    random.setstate(rng["rng_python_state"])
    ...
    optimizer = AdamW(filter(lambda p: p.requires_grad,student.parameters()), lr= LR, weight_decay= 0.01)
...
while step < Max_steps:
    ...
    if step % Log_every == 0:
        ...
        for name, param in student.named_parameters():
            if "lora_A" in name:
                wandb.log({f"adapter_norm/{name}": param.data.norm().item()})
                break
...
merged.save_pretrained(f"{final_path}_merged")
