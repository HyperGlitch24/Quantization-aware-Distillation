    ...
    for name, module in model.named_children():
        if isinstance(module, nn.Linear):
            ...
            with deepspeed.zero.GatheredParameters(
                ...
                with torch.no_grad():
                    weight_copy = module.weight.data.clone().to(torch.bfloat16)
                    bias_copy = (
                        module.bias.data.clone().to(torch.bfloat16)
                        if module.bias is not None else None
                    )
            new_layer.weight = nn.Parameter(weight_copy)
            if bias_copy is not None:
                new_layer.bias = nn.Parameter(bias_copy)
...
Ckpt_dir = "./poc_checkpoints_7B_4bit_QAT_Qwen_codeparrot"
...
if latest_ckpt is not None:
    ...
    optimizer = AdamW(student.parameters(), lr=LR, weight_decay=0.00)
    ...
    for group in student_engine.optimizer.optimizer.param_groups:
        for p in group['params']:
            state = student_engine.optimizer.optimizer.state.get(p, {})
    ...
    if is_main_process:
        for i in range(torch.cuda.device_count()):
            ...
            print(f"GPU {i}: {mem:.2f}GB allocated after resume")

    print(f"Rank {local_rank} memory after checkpoint load: "
...
#save
if step >= Max_steps:
    # Save one more DeepSpeed checkpoint at true final step
    save_checkpoint(step, student_engine, scheduler, all_indices, Ckpt_dir)
    dist.barrier(device_ids=[local_rank])
    ...
    if is_main_process:
        ...
        hf_model.load_state_dict(state_dict_bf16, strict=False)
        hf_model.save_pretrained(f"{Ckpt_dir}/final")
        print(f"✅ Correctly saved final model to {Ckpt_dir}/final")
