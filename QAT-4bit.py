...
class FakeQuantLinear(nn.Linear):
    def __init__(self, in_features, out_features, bias=True, num_bits=4):
        super().__init__(in_features, out_features, bias)
        self.num_bits = num_bits

    def fake_quantize(self, weight: torch.Tensor) -> torch.Tensor:
        n_levels = 2 ** (self.num_bits - 1) - 1   # 7 for INT4, 127 for INT8
...
def replace_with_fake_quant_linear(model: nn.Module, num_bits: int = 4):
...
Student_path = "./StudentModel/Qwen2.5-Coder-3B/"
Cache_dir = "./poc_cache_Qwen_codeparrot_32B"
Ckpt_dir = "./poc_checkpoints_3B_QAT4bit_Qwen_32B"
...
if latest_ckpt is not None:
    ...
    rng = torch.load(
        os.path.join(latest_ckpt, "rng_states.pt"),
        weights_only=False,
    )
    torch.set_rng_state(rng["rng_torch"])
    torch.cuda.set_rng_state(rng["rng_cuda"])
...
p_before = next(student.parameters()).detach().clone()    
...
while step < Max_steps:
    ...
    scheduler.step()
    if step == start_step:
        delta = (next(student.parameters()) - p_before).abs().max().item()
        print(f"[verify] max param change after first step: {delta:.8f}")
        if delta == 0.0:
            print("[verify] WARNING: no weight update — optimizer is not bound "
